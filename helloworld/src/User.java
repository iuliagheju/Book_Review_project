...
public abstract class User implements Savable, Comparable<User> {
    ...
    public int compareTo(User other) {
        return Integer.compare(this.booksRead.size(), other.booksRead.size());
    }
    // Adds a book to the booksRead list
    public void addBook(Book book) {
        booksRead.add(book);
        reviews.add(new ArrayList<>()); // Initialize an empty review list for the new book
    }
}